...
import React, { useRef, useEffect, type ReactNode, type MouseEventHandler } from 'react';
...
export function SpecularButton({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: Event) => {
      ...
      const nx = (pe.clientX - cx) / (rect.width / 2);
      const ny = (cy - pe.clientY) / (rect.height / 2);
    ...
    const update = (now: number) => {
      ...
      const steer = isHovered && p.followMouse && pointerAngle != null;
      const targetAngle = (steer && pointerAngle != null) ? pointerAngle : idleAngle;
      const diff = ((targetAngle - angle + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
      // Smoother spring-like easing dampening
      angle += diff * (1 - Math.exp(-dt * 12));

      // Shine fades in strictly on hover
      const brightTarget = isHovered ? 1 : 0;
