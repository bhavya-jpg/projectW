...
const InfiniteSlider = memo(function InfiniteSlider({
  ...
  useEffect(() => {
    ...
    // Because container contains 2 exact copies of children ({children}{children}),
    // 1 full set of children has size: halfSize = (size - gap) / 2.
    // The exact loop distance for 1 set of items including its trailing gap is:
    const stepSize = (size + gap) / 2;

    const from = reverse ? -stepSize : 0;
    const to = reverse ? 0 : -stepSize;
    ...
    if (isTransitioning) {
      controls = animate(translation, [translation.get(), to], {
        ease: "linear",
        duration:
          currentDuration * Math.abs((translation.get() - to) / stepSize),
        onComplete: () => {
